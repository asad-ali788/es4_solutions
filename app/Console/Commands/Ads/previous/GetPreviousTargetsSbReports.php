...
class GetPreviousTargetsSbReports extends Command
{
    protected $signature = 'app:get-previous-targets-sb-reports';
    protected $description = 'Fetch and save completed Amazon Sponsored Brands Targets reports for past 3 months';
    ...
    public function handle(AmazonAdsService $client)
    {
        ...
        foreach ($countries as $country) {
            Log::info("📥 Processing {$country} Sponsored Brands Targets Previous Reports...");
            ...
            if ($logs->isEmpty()) {
                Log::info("ℹ️ No pending SB Targets reports found for {$country}.");
                continue;
            }
            ...
            foreach ($logs as $reportLog) {
                ...
                try {
                    ...
                    if (in_array($responseData['status'], ['COMPLETED', 'SUCCESS'])) {
                        ...
                        Log::info("✅ [$country][$dateStr] SB Targets Report saved and marked as COMPLETED.");
                    ...
                    Log::error("💥 [$country][$dateStr] SB Targets Report error: " . $e->getMessage(), [
        ...
        Log::info("🚀 Completed all SB Targets historical reports for US & CA.");
    }
}
