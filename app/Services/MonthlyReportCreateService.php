...
class MonthlyReportCreateService
{
    /**
    ...
    /**
     * Creates a Monthly report request via Amazon SP API and logs it to the database.
     *
    ...
    public function createReport(string $reportType, array $marketplaceIds): array
    {
        ...
        try {
            ...
            if ($exists) {
                return [
                    'success' => false,
                    'message' => '⚠️Monthly Report already created for the given date and marketplace.',
                ];
            }
            ...
            Log::error('Amazon SP API Monthly Report Create Error : ' . $e->getMessage());

            return [
                'success' => false,
                'message' => '❌ Exception while creating Monthly report.',
                'error'   => $e->getMessage(),
            ];
        }
    }
