<div>
    @if ($show)
        ...
        {{-- Modal shell (pure Livewire, no Bootstrap JS needed) --}}
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content livewire-modal-anim">
                    <div class="modal-header">
                        <h5 class="modal-title">Keyword Optimization Recommendations</h5>
                        {{-- Close button uses Livewire --}}
                        <button type="button" class="btn-close" wire:click="close" aria-label="Close"></button>
                    </div>
                    {{-- Lazy load rules after modal appears --}}
                    <div class="modal-body">

                        <div class="alert alert-warning mt-3" role="alert">
                            <strong>⚠️ Disclaimer:</strong> AI-generated recommendations may not always be accurate or
                            suitable for every case. Please review carefully before applying changes.
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            @can('amazon-ads.keyword-cron-rules.update')
                                <a href="{{ route('admin.ads.performance.rules.keyword.index') }}">
                                    <button type="button"
                                        class="btn btn-success btn-sm btn-rounded waves-effect waves-light">
                                        <i class="mdi mdi-pencil label-icon"></i> Edit Keyword Rules
                                    </button>
                                </a>
                            @endcan
                        </div>
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped align-middle">
                                <thead class="table-sm table-light">
                                    <tr>
