                ...
                <div class="d-flex align-items-center gap-3 flex-wrap">
                    <h4 class="mb-0">
                        ...
                        <span class="text-success">
                            {{ $items->first() ? '(' . $items->first()->order->order_number . ')' : 'No Items available for this Order' }}
                        </span>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body" style="overflow: visible;">
                    <div class="row mb-2">
                        <div class="col-sm-4">
                            <form method="GET" action="{{ route('admin.purchaseOrder.items', $orderId) }}"
                        ...
                        <div class="col-sm-8 text-end">
                            @if (isset($items) && count($items) > 0)
                                <a href="{{ asset('assets/example/example_updatepurchaseorder.xlsx') }}" download>
                                ...
                                <form id="excelUploadForm"
                                    action="{{ route('admin.purchaseOrder.updatePurchaseOrderItems', $orderId) }}"
                            ...
                            @endif
                            <a href="{{ route('admin.purchaseOrder.itemCreate', $orderId) }}">
                                <button
                                    class="btn btn-success btn-rounded waves-effect waves-light mb-2 me-2 addCustomers-modal">
                                    <i class="mdi mdi-plus me-1"></i> New Order Item
                                </button>
                            </a>
                        </div>
                        <!-- end col-->
                                ...
                                @if ($items->count())
                                    @foreach ($items as $index => $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->total_price !== null ? number_format($item->total_price, 2) : '--' }}
                                            </td>
                                            <td>
                                            ...
                                            {{-- <td>{{ $item->updated_at ? \Carbon\Carbon::parse($item->updated_at)->format('d-m-Y H:i') : '-' }} --}}
                                            </td>
                                            <td>
                                                <!-- Action Dropdown -->
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu dropdown-menu-end">
                                                        <li>
                                                            <a href="{{ route('admin.purchaseOrder.itemEdit', $item->id) }}"
                                                                class="dropdown-item">
                                                                <i
                                                        ...
                                                        <li>
                                                            <form
                                                                action="{{ route('admin.purchaseOrder.itemDelete', $item->id) }}"
